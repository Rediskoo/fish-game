...
import { useBreeding, useBreedingAction, useInviteBreeding, useStartBreeding } from "@/features/breeding/use-breeding";
...
export function BreedingScreen() {
  ...
  const invite = useInviteBreeding();
  ...
  const [invitationFishId, setInvitationFishId] = useState("");
  ...
  return <div className="space-y-4 overflow-x-hidden p-3 pb-8 sm:p-4">
    ...
    {(breeding.data?.invitations ?? []).map((invitation) => <Panel key={invitation.id} className="space-y-3 border-pink-200/25 bg-pink-300/10"><div className="font-black text-pink-100">{invitation.direction === "received" ? `${invitation.friendName} зовёт тебя стать вторым родителем` : `Ждём рыбу от ${invitation.friendName}`}</div><div className="text-xs text-cyan-100/65">Первая рыба: {invitation.parent.displayName} · {invitation.parent.species}</div>{invitation.direction === "received" ? <div className="space-y-2"><select aria-label="Твоя рыба для совместного скрещивания" className="h-11 w-full rounded-xl border border-pink-200/20 bg-slate-950/70 px-3 text-sm" value={invitationFishId} onChange={(event) => setInvitationFishId(event.target.value)}><option value="">Выбери свою взрослую рыбу</option>{fish.filter((candidate) => candidate.lifeStage === "ADULT" && !candidate.breedingLocked && Boolean(findHybrid(invitation.parent.species, breedingSpeciesKey(candidate)))).map((candidate) => <option key={candidate.id} value={candidate.id}>{candidate.name} · {candidate.displayName}</option>)}</select><Button className="w-full bg-pink-300" disabled={!invitationFishId || action.isPending} onClick={() => action.mutate({ action: "accept-parent-invite", invitationId: invitation.id, parentFishId: invitationFishId })}>Добавить свою рыбу и начать</Button></div> : null}<Button className="w-full bg-slate-950/45" disabled={action.isPending} onClick={() => action.mutate({ action: "cancel-parent-invite", invitationId: invitation.id })}>{invitation.direction === "received" ? "Отказаться" : "Отменить приглашение"}</Button></Panel>)}
    {(breeding.data?.jobs ?? []).filter((job) => !["completed", "cancelled"].includes(job.status)).map((job) => <Panel key={job.id} className="relative overflow-hidden p-0">
      ...
      <div className="relative space-y-3 p-4"><div className="flex justify-between gap-2"><div><div className="text-xs uppercase text-cyan-100/60">{stageLabels[job.lifeStage]}</div><div className="font-black text-cyan-50">{job.parentA.displayName} × {job.parentB.displayName}</div>{job.collaboration?.accepted ? <div className="mt-1 text-xs font-bold text-pink-100">Выращиваете вместе с {job.collaboration.friendName}</div> : null}</div><span className="h-fit rounded-full bg-slate-950/55 px-2 py-1 text-[10px] uppercase">{job.rarity}</span></div>
      ...
      <div><div className="mb-1 flex justify-between text-[10px] text-cyan-100/60"><span>Общий прогресс взросления</span><span>{(developmentProgress(job, new Date(displayNow)) * 100).toFixed(2)}%</span></div><div className="relative h-3 overflow-hidden rounded-full bg-slate-950/55"><div className="h-full rounded-full bg-gradient-to-r from-cyan-300 via-sky-300 to-emerald-300 transition-[width] duration-1000 ease-linear" style={{ width: `${developmentProgress(job, new Date(displayNow)) * 100}%` }} /><div className="absolute inset-y-0 w-16 animate-[pulse_1.5s_ease-in-out_infinite] bg-gradient-to-r from-transparent via-white/35 to-transparent" /></div></div>
      <div className="flex justify-between text-xs text-cyan-100/75"><span className="flex items-center gap-1"><Clock3 className="h-4 w-4" />{job.lifeStage === "adult" ? "Готово" : remaining(job.adultAt, displayNow)}</span><span>{job.speedupsUsed}</span></div>
      {job.lifeStage !== "adult" && (!job.collaboration || job.collaboration.isOwner || job.collaboration.accepted) ? <div className="space-y-2 rounded-2xl border border-amber-200/20 bg-slate-950/45 p-3"><div className="flex items-center justify-between text-xs"><span className="font-black text-amber-100">Ускорение развития</span><span className="text-cyan-100/60">использовано: {job.speedupsUsed}</span></div>{job.lifeStage === "egg" || job.lifeStage === "embryo" ? <Button className="w-full bg-cyan-300 text-slate-950" disabled={action.isPending || !breeding.data?.inventory.eggIncubator} onClick={() => action.mutate({ jobId: job.id, action: "incubate" })}><Zap className="h-4 w-4" /> Инкубатор −1 час · {breeding.data?.inventory.eggIncubator ?? 0}</Button> : null}<Button className="w-full bg-amber-300 text-slate-950" disabled={action.isPending || !breeding.data?.inventory.fryFood} onClick={() => action.mutate({ jobId: job.id, action: "speed-up" })}><Zap className="h-4 w-4" /> Корм для мальков −2 часа · {breeding.data?.inventory.fryFood ?? 0}</Button><Button className="w-full bg-emerald-300 text-slate-950" disabled={action.isPending || !breeding.data?.inventory.nurseryConditioner} onClick={() => action.mutate({ jobId: job.id, action: "condition" })}><Sparkles className="h-4 w-4" /> Кондиционер −30 минут · {breeding.data?.inventory.nurseryConditioner ?? 0}</Button><p className="text-[11px] text-cyan-100/55">Корм для мальков доступен на любой незавершённой стадии и всегда ускоряет ближайшие сроки.</p></div> : null}
    ...
    <Panel className="space-y-4"><div className="grid grid-cols-[1fr_auto_1fr] items-center gap-2"><Slot fish={parentA} label="Родитель A" /><Sparkles className="h-5 w-5 text-amber-200" /><Slot fish={parentB} label="Родитель B" /></div>
      ...
      {parentA && !parentB ? <div className="space-y-2 rounded-2xl border border-pink-200/20 bg-pink-300/10 p-3"><select aria-label="Друг для второй рыбы" className="h-11 w-full rounded-xl border border-pink-200/20 bg-slate-950/70 px-3 text-sm" value={inviteFriendId} onChange={(event) => setInviteFriendId(event.target.value)}><option value="">Выбери друга со второй рыбой</option>{(friends.data?.friends ?? []).map((friend) => <option key={friend.id} value={friend.id}>{friend.profileName ?? friend.firstName ?? friend.username ?? friend.telegramId}</option>)}</select><Button className="w-full bg-pink-300" disabled={!inviteFriendId || invite.isPending || !breeding.data?.inventory.spawningNest} onClick={() => invite.mutate({ parentFishId: parentA.id, friendId: inviteFriendId, idempotencyKey: crypto.randomUUID() }, { onSuccess: () => setSelected([]) })}><Heart className="h-4 w-4" /> Пригласить друга · гнёзд {breeding.data?.inventory.spawningNest ?? 0}</Button><p className="text-[11px] text-cyan-100/60">Друг сам выберет вторую рыбу. Гнездо спишется только после его согласия.</p></div> : <Button className="w-full" disabled={!hybrid || !parentA || !parentB || start.isPending || !breeding.data?.inventory.spawningNest} onClick={() => parentA && parentB && start.mutate({ parentAId: parentA.id, parentBId: parentB.id, idempotencyKey: crypto.randomUUID() })}><Egg className="h-4 w-4" /> Начать · гнёзд {breeding.data?.inventory.spawningNest ?? 0}</Button>}
      {start.error || invite.error || action.error ? <p className="text-sm text-rose-200">{(start.error ?? invite.error ?? action.error)?.message}</p> : null}</Panel>
