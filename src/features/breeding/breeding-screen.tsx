...
const stageLabels = { egg: "РРєСЂР°", embryo: "Р­РјР±СЂРёРѕРЅС‹", hatching: "Р’С‹Р»СѓРїР»РµРЅРёРµ", fry: "РњР°Р»СЊРєРё", baby: "РњР°Р»С‹С€", adult: "Р’Р·СЂРѕСЃР»Р°СЏ СЂС‹Р±Р°" };
...
function remaining(target: string, now: number) {
  ...
  return `${Math.floor(seconds / 3600)}С‡ ${Math.floor(seconds % 3600 / 60)}Рј ${seconds % 60}СЃ`;
}
...
export function BreedingScreen() {
  ...
  return <div className="space-y-4 overflow-x-hidden p-3 pb-8 sm:p-4">
    <header className="pt-20"><div className="flex items-center gap-3"><span className="grid h-12 w-12 place-items-center rounded-2xl bg-amber-300/15 text-amber-100"><Egg /></span><div><h1 className="text-2xl font-black text-cyan-50 sm:text-3xl">РЎРєСЂРµС‰РёРІР°РЅРёРµ</h1><p className="text-xs text-cyan-100/60">РџРёС‚РѕРјРЅРёРє РіРёР±СЂРёРґРЅС‹С… СЂС‹Р±</p></div></div></header>
    {(breeding.data?.jobs ?? []).filter((job) => !["completed", "cancelled"].includes(job.status)).map((job) => <Panel key={job.id} className="relative overflow-hidden p-0">
      ...
      <div className="relative space-y-3 p-4"><div className="flex justify-between gap-2"><div><div className="text-xs uppercase text-cyan-100/60">{stageLabels[job.lifeStage]}</div><div className="font-black text-cyan-50">{job.parentA.displayName} Г— {job.parentB.displayName}</div></div><span className="h-fit rounded-full bg-slate-950/55 px-2 py-1 text-[10px] uppercase">{job.rarity}</span></div>
      ...
      <div className="flex justify-between text-xs text-cyan-100/75"><span className="flex items-center gap-1"><Clock3 className="h-4 w-4" />{job.lifeStage === "adult" ? "Р“РѕС‚РѕРІРѕ" : remaining(job.adultAt, displayNow)}</span><span>{job.speedupsUsed}/3</span></div>
      {job.lifeStage === "fry" || job.lifeStage === "baby" ? <Button className="w-full" disabled={action.isPending || !breeding.data?.inventory.fryFood || job.speedupsUsed >= 3} onClick={() => action.mutate({ jobId: job.id, action: "speed-up" })}><Zap className="h-4 w-4" /> Р‘С‹СЃС‚СЂРѕРµ РІР·СЂРѕСЃР»РµРЅРёРµ В· РєРѕСЂРј {breeding.data?.inventory.fryFood ?? 0}</Button> : null}
      {job.lifeStage === "adult" ? <Button className="w-full bg-emerald-300" disabled={action.isPending} onClick={() => action.mutate({ jobId: job.id, action: "claim" })}><Check className="h-4 w-4" /> Р—Р°Р±СЂР°С‚СЊ СЂС‹Р±Сѓ</Button> : null}</div>
    </Panel>)}
    <Panel className="space-y-4"><div className="grid grid-cols-[1fr_auto_1fr] items-center gap-2"><Slot fish={parentA} label="Р РѕРґРёС‚РµР»СЊ A" /><Sparkles className="h-5 w-5 text-amber-200" /><Slot fish={parentB} label="Р РѕРґРёС‚РµР»СЊ B" /></div>
      {parentA && parentB ? <div className="rounded-2xl bg-slate-950/35 p-3">{hybrid ? <div className="flex items-center gap-3"><img className="h-20 w-20 object-contain" src={aquariumAssets.breeding.offspring(hybrid.key)} alt="" /><div><div className="flex items-center gap-1 font-black"><Dna className="h-4 w-4" />{hybrid.key}</div><div className="text-xs text-cyan-100/65">{hybrid.config.rarity} В· 12 С‡Р°СЃРѕРІ</div></div></div> : <p className="text-sm text-amber-100">Р”Р»СЏ СЌС‚РѕР№ РїР°СЂС‹ РЅРµС‚ РіРѕС‚РѕРІРѕРіРѕ РІРёР·СѓР°Р»СЊРЅРѕРіРѕ РІР°СЂРёР°РЅС‚Р°.</p>}</div> : null}
      <Button className="w-full" disabled={!hybrid || !parentA || !parentB || start.isPending || !breeding.data?.inventory.spawningNest} onClick={() => parentA && parentB && start.mutate({ parentAId: parentA.id, parentBId: parentB.id, idempotencyKey: crypto.randomUUID() })}><Egg className="h-4 w-4" /> РќР°С‡Р°С‚СЊ В· РіРЅС‘Р·Рґ {breeding.data?.inventory.spawningNest ?? 0}</Button>
      {start.error || action.error ? <p className="text-sm text-rose-200">{(start.error ?? action.error)?.message}</p> : null}</Panel>
    <div className="space-y-3"><div className="flex gap-2"><label className="relative flex-1"><Search className="absolute left-3 top-3 h-4 w-4 text-cyan-100/45" /><input className="h-10 w-full rounded-xl border border-cyan-100/15 bg-slate-950/35 pl-9 pr-3 text-sm" placeholder="РРјСЏ РёР»Рё РІРёРґ" value={query} onChange={(event) => setQuery(event.target.value)} /></label><button type="button" onClick={() => setFavorites(!favorites)} className={`h-10 w-10 rounded-xl border ${favorites ? "bg-pink-300/15 text-pink-200" : "text-cyan-100/60"}`}><Heart className="mx-auto h-4 w-4" /></button></div>
