...
export function BreedingScreen() {
  ...
  return <div className="space-y-4 overflow-x-hidden p-3 pb-8 sm:p-4">
    ...
    {(breeding.data?.jobs ?? []).filter((job) => !["completed", "cancelled"].includes(job.status)).map((job) => <Panel key={job.id} className="relative overflow-hidden p-0">
      ...
      <CollaborationControls job={job} friends={friends.data?.friends ?? []} friendId={inviteFriendId} setFriendId={setInviteFriendId} pending={action.isPending} act={(input) => action.mutate(input)} />
...
function CollaborationControls({ job, friends, friendId, setFriendId, pending, act }: { job: BreedingJobView; friends: FriendView[]; friendId: string; setFriendId: (id: string) => void; pending: boolean; act: (input: { jobId: string; action: "invite" | "accept"; friendId?: string }) => void }) {
  if (job.collaboration) return <div className="rounded-2xl border border-pink-200/20 bg-pink-300/10 p-3 text-xs"><div className="font-black text-pink-100">Совместно с {job.collaboration.friendName}</div><div className="mt-1 text-cyan-100/65">{job.collaboration.accepted ? "Оба можете ускорять развитие — предмет платит тот, кто нажал." : job.collaboration.isOwner ? "Приглашение отправлено" : "Тебя пригласили выращивать дитятко"}</div>{!job.collaboration.accepted && !job.collaboration.isOwner ? <Button className="mt-2 w-full bg-pink-300" disabled={pending} onClick={() => act({ jobId: job.id, action: "accept" })}>Принять приглашение</Button> : null}</div>;
  if (job.lifeStage === "adult") return null;
  return <div className="space-y-2 rounded-2xl border border-pink-200/25 bg-slate-950/45 p-3"><div className="font-black text-pink-100">Позвать друга выращивать рыбку</div>{friends.length ? <div className="flex gap-2"><select aria-label="Друг для совместного выращивания" className="min-w-0 flex-1 rounded-xl border border-pink-200/20 bg-slate-950/80 px-3 text-xs" value={friendId} onChange={(event) => setFriendId(event.target.value)}><option value="">Выбери друга</option>{friends.map((friend) => <option key={friend.id} value={friend.id}>{friend.profileName ?? friend.firstName ?? friend.username ?? friend.telegramId}</option>)}</select><Button className="bg-pink-300 px-4" disabled={!friendId || pending} onClick={() => act({ jobId: job.id, action: "invite", friendId })}>Пригласить</Button></div> : <p className="text-xs text-cyan-100/60">Сначала добавь человека в друзья в профиле.</p>}</div>;
}
