...
const stageLabels = { egg: "РРєСЂР°", embryo: "Эмбрионы", hatching: "Вылупление", fry: "Мальки", baby: "Малыш", adult: "Взрослая рыба" };
...
export function BreedingScreen() {
  ...
  return <div className="space-y-4 overflow-x-hidden p-3 pb-8 sm:p-4">
    ...
    <div className="space-y-3"><div className="flex gap-2"><label className="relative flex-1"><Search className="absolute left-3 top-3 h-4 w-4 text-cyan-100/45" /><input className="h-10 w-full rounded-xl border border-cyan-100/15 bg-slate-950/35 pl-9 pr-3 text-sm" placeholder="РРјСЏ или вид" value={query} onChange={(event) => setQuery(event.target.value)} /></label><button type="button" onClick={() => setFavorites(!favorites)} className={`h-10 w-10 rounded-xl border ${favorites ? "bg-pink-300/15 text-pink-200" : "text-cyan-100/60"}`}><Heart className="mx-auto h-4 w-4" /></button></div>
