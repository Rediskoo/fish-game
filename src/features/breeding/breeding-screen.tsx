...
function CollaborationControls({ job, friends, friendId, setFriendId, pending, act }: { job: BreedingJobView; friends: FriendView[]; friendId: string; setFriendId: (id: string) => void; pending: boolean; act: (input: { jobId: string; action: "invite" | "accept"; friendId?: string }) => void }) {
  ...
  return <div className="flex gap-2"><select className="min-w-0 flex-1 rounded-xl border border-cyan-100/15 bg-slate-950/60 px-3 text-xs" value={friendId} onChange={(event) => setFriendId(event.target.value)}><option value="">Пригласить друга</option>{friends.map((friend) => <option key={friend.id} value={friend.id}>{friend.profileName ?? friend.firstName ?? friend.username ?? friend.telegramId}</option>)}</select><Button className="bg-pink-300" disabled={!friendId || pending} onClick={() => act({ jobId: job.id, action: "invite", friendId })}>Позвать</Button></div>;
}
