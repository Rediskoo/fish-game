...
function visual(job: BreedingJobView) {
  ...
  return aquariumAnimations.babySwim(job.hybridKey).animatedFile ?? aquariumAssets.breeding.hybrid(job.hybridKey);
}
...
export function BreedingScreen() {
  ...
  return <div className="space-y-4 overflow-x-hidden p-3 pb-8 sm:p-4">
    ...
    {(breeding.data?.jobs ?? []).filter((job) => !["completed", "cancelled"].includes(job.status)).map((job) => <Panel key={job.id} className="relative overflow-hidden p-0">
      ...
      {job.lifeStage === "fry" || job.lifeStage === "baby" ? <Button className="w-full" disabled={action.isPending || !breeding.data?.inventory.fryFood || job.speedupsUsed >= 3} onClick={() => action.mutate({ jobId: job.id, action: "speed-up" })}><Zap className="h-4 w-4" /> Быстрое взросление · корм {breeding.data?.inventory.fryFood ?? 0}</Button> : null}
    ...
    <Panel className="space-y-4"><div className="grid grid-cols-[1fr_auto_1fr] items-center gap-2"><Slot fish={parentA} label="Родитель A" /><Sparkles className="h-5 w-5 text-amber-200" /><Slot fish={parentB} label="Родитель B" /></div>
      {parentA && parentB ? <div className="rounded-2xl bg-slate-950/35 p-3">{hybrid ? <div className="flex items-center gap-3"><img className="h-20 w-20 object-contain" src={aquariumAssets.breeding.hybrid(hybrid.key)} alt="" /><div><div className="flex items-center gap-1 font-black"><Dna className="h-4 w-4" />{hybrid.key}</div><div className="text-xs text-cyan-100/65">{hybrid.config.rarity} · 12 часов</div></div></div> : <p className="text-sm text-amber-100">Для этой пары нет готового визуального варианта.</p>}</div> : null}
