...
export function useSharedAquariumAction() { const client = useQueryClient(); return useMutation({ mutationFn: (input: { aquariumId: string; action: "feed" | "clean" | "rename"; fishId?: string; name?: string }) => api<SharedAquariumView[]>("/api/shared-aquariums", { method: "PATCH", body: JSON.stringify(input) }), onSuccess: (data) => { client.setQueryData(["shared-aquariums"], data); void client.invalidateQueries({ queryKey: ["snapshot"] }); } }); }
