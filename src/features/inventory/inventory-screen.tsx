    ...
    <div className="space-y-4 p-4">
      ...
      <div className="grid gap-3">
        {player.data?.fish.map((fish) => {
          const hungerPercent = Math.min(100, Math.round((fish.hunger / fish.maxHunger) * 100));
          const isFull = fish.hunger <= 0;
            ...
            <Panel key={fish.id} className="flex items-center justify-between gap-3">
              <div className="min-w-0">
                ...
                <div className="text-sm text-cyan-100/60">
                  Голод {fish.hunger}/{fish.maxHunger} ({hungerPercent}%) · доход +{fish.incomePerSecond.toFixed(1)}/сек
                </div>
              </div>
              <Button disabled={feed.isPending || food <= 0 || isFull} onClick={() => feed.mutate(fish.id)}>
