...
export function ensureLatestSchema(db: PrismaClient) {
  schemaPromise ??= (async () => {
    ...
    await db.$executeRawUnsafe(`ALTER TABLE "shared_aquariums" ADD COLUMN IF NOT EXISTS "backgroundId" TEXT NOT NULL DEFAULT 'deep-lagoon'`);
    await db.$executeRawUnsafe(`ALTER TABLE "shared_aquariums" ADD COLUMN IF NOT EXISTS "decor" JSONB NOT NULL DEFAULT '[]'`);
    await db.$executeRawUnsafe(`ALTER TABLE "shared_aquariums" ADD COLUMN IF NOT EXISTS "ownedItemIds" JSONB NOT NULL DEFAULT '[]'`);
  })().catch((error) => {
    schemaPromise = null;
    throw error;
  });
